...
SCANVI_DIR = Path(__file__).resolve().parent / "scanvi_celltype_split"
...
# Target vector string and index encoding
...
def generate_curves(y_true, y_prob, output_dir, prefix=""):
    ...
    ax.set_title(f"{prefix.upper()} ROC Curve [scANVI Baseline]", fontsize=13, fontweight='bold')
    ...
    ax.set_title(f"{prefix.upper()} Precision-Recall (AUPR) Curve [scANVI Baseline]", fontsize=13, fontweight='bold')
    ax.legend(loc="upper left", bbox_to_anchor=(1.02, 1.0), fontsize=8, ncol=2, frameon=True)
    plt.tight_layout()
...
for fold, (train_idx, test_idx) in enumerate(skf.split(adata.X, y_cell)):
    print(f"\n--- Processing scANVI Classifier Fold {fold+1}/5 ---")
    ...
    y_true_fold = y_cell[test_idx]
    y_pred_fold = le_cell.transform(y_pred_str)
    ...
    with open(os.path.join(fold_dir, "classification_report.txt"), "w") as f:
        f.write(classification_report(y_true_fold, y_pred_fold, labels=np.arange(n_classes), target_names=le_cell.classes_, zero_division=0))
...
np.save(os.path.join(SCANVI_DIR, "oof_true.npy"), y_cell)
...
calculate_and_save_metrics(y_cell, oof_preds, oof_probs, SCANVI_DIR, prefix="overall_")
generate_curves(y_cell, oof_probs, SCANVI_DIR, prefix="overall_")
