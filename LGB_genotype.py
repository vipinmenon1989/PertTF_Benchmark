...
# Target vector encoding using original genotype metadata
le_geo = LabelEncoder()
...
# ==============================================================================
print("\n=== EXECUTING PIPELINE: 5-FOLD STRATIFIED RANDOM CV FOR GENOTYPE ===")
skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)

# Global tracking structures for out-of-fold pooled metrics matrix initialization
...
for fold, (train_idx, test_idx) in enumerate(skf.split(X_df, y_geo)):
    print(f"\n--- Processing Genotype CV Fold {fold+1}/5 ---")
    X_train_rand, Y_train_rand = X_df.iloc[train_idx], y_geo[train_idx]
    X_test_rand, Y_test_rand = X_df.iloc[test_idx], y_geo[test_idx]
    ...
    # Accumulate out-of-fold evaluation predictions
    oof_preds[test_idx] = y_pred_rand
    oof_probs[test_idx] = y_prob_rand

    fold_dir = os.path.join(GENOTYPE_DIR, f"fold_{fold+1}")
    os.makedirs(fold_dir, exist_ok=True)

    with open(os.path.join(fold_dir, "classification_report.txt"), "w") as f:
        f.write(classification_report(Y_test_rand, y_pred_rand, labels=np.arange(n_classes), target_names=le_geo.classes_, zero_division=0))
...
with open(os.path.join(GENOTYPE_DIR, "overall_classification_report.txt"), "w") as f:
    f.write(classification_report(y_geo, oof_preds, labels=np.arange(n_classes), target_names=le_geo.classes_, zero_division=0))

np.save(os.path.join(GENOTYPE_DIR, "oof_true.npy"), y_geo)
np.save(os.path.join(GENOTYPE_DIR, "oof_pred.npy"), oof_preds)
np.save(os.path.join(GENOTYPE_DIR, "oof_prob.npy"), oof_probs)

# Execute aggregated performance metric extraction
calculate_and_save_metrics(y_geo, oof_preds, oof_probs, GENOTYPE_DIR, prefix="overall_")
generate_curves(y_geo, oof_probs, GENOTYPE_DIR, prefix="overall_")

print(f"\nAll tasks finalized. Master cross-validation logs and curves compiled directly under: {GENOTYPE_DIR}")
